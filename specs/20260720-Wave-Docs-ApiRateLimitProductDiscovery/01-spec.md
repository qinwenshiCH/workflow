...
| 场景 | 典型客户 | 核心操作 | 对 API 的依赖特征 |
| --- | --- | --- | --- |
| **A) 自动化运维** | 平台工程团队 | 创建/更新/删除 Dashboard、Chart、实验；在 CI/CD 中管理资源配置 | 写操作为主；需要可预测的速率和稳定性；低延迟敏感度 |
| **B) 数据集成** | 数据工程/分析团队 | 查询数据、导出报表、接入数据管道或 AI 工具 | 读操作为主；查询成本高且波动大；响应时间容忍度低 |
| **C) 自定义应用** | 高阶客户/ISV | CRUD + 查询混合，封装为面向自身业务的界面 | 读写混合；需要灵活的 scope 和 token 管理；对错误语义敏感 |

### 1.3 范围：纳入本次产品设计的 API 面

基于 Wave OpenAPI 路由梳理，以下资源由 Account API Token 可访问，纳入本次限流设计：
...
> **关于 Organization 的角色**：本阶段认为 organization 是商业归属单位（是否开通、套餐映射、长期用量汇总），project 是运行时限流单位。如果产品确认后需要 organization 总容量上限（防止多 project 放大），可在同一模型上增加但不在第一版锁死。
...
Account API Token 是面向开发者的机器凭证，通过 `Authorization: Bearer <token>` 传递。请求依次经过 4 层保护，每层覆盖不同的风险场景。
...
| #2 | **账号** | 账号聚合速率 | per account | 同一账号下多枚 token 叠加超出账号容量 | 否，由 max_projects + #3 间接覆盖 |
| #3 | **项目** | 项目聚合速率（加权） | per project | 多 token 多路径并发叠加超出套餐承诺 | ✅ 套餐容量 |
| #4 | **平台** | 全局 RPS | 全站 | 基础设施过载，保护 Redis 单例 | 否，硬编码保护线 |

**各层设计说明：**

- **#1 单 token 突发速率 (10/s 全局统一)**：不区分套餐。如果 per-token 按套餐划分，客户可创建大量低套餐 token 叠加超出项目容量（例：PRO 的 project=300/s，创建 30 个 token 各 60/s → 叠加远超 300/s）。10/s 是安全阈值，不是容量承诺。
- **#2 账号聚合（间接覆盖）**：不设独立 Redis 计数器。由 token 创建上限（`MaxTokensPerUser=3`）+ #1 per-token 突发速率间接覆盖。每个账号最多 3 枚 token，每枚上限 10/s → 账号级最大聚合约 30/s。如需更大容量，可提升 token 创建上限或购买更高套餐。
- **#3 项目聚合速率（加权）**：这是**真正的套餐容量**。所有指向同一项目的 token 共享加权容量池。加权 cost 参考 Wave 现有 [`resolveRequestCost`](</Users/wenshiqin/wave-worktrees/api_qps/apps/web/service/account/apitoken/ratelimit.go:267>)——UBA 查询/下载=3 单位、AB report=2 单位、CRUD=1 单位。
- **#4 全局 RPS (2000/s)**：全站硬上限，保护 Redis 和下游服务。不区分套餐，不在商业合同中承诺。
...
| 2 | **账号** | 账号聚合速率 | per account | — | — | — | ✅ 由 MaxTokensPerUser=3 + per-token 10/s 间接覆盖 |
| 3 | **项目** | 项目聚合速率（加权） | per project | **50/s** | **300/s** | **1,500/s** | ❌ 未实现 |
| 4 | **平台** | 全局 RPS | 全站 | **2,000/s** | **2,000/s** | **2,000/s** | ✅ 硬编码 2000/s |
...
>
> **#1 与 #3 的关系**：Project 允许多个 token，#1 保证单枚 token 不异常突发，#3 保证所有 token 之和不超过套餐容量。#1 应 ≤ #3 / 预期最小 token 数。例：PLUS project=50/s，预期最多 5 个 token → #1 ≤ 10/s ✅。

#### 5.1.3 竞品对比

| 维度 | Wave | PostHog | Amplitude |
|------|------|---------|-----------|
| **保护层级** | 4 层：token → 账号 → 项目(加权) → 全局 | 2 层：key → team | 2–3 层：token → project → org（因 token 类型而异） |
| **时间窗口** | 单层 burst | burst(480/min) + sustained(4800/hour) | 100/s + 100,000/day（双窗口） |
| **速率配置粒度** | 每 project（加权聚合） | 每 key + 每 team | 每 project + 每日 quota |
| **Plan 差异化** | project_agg 50/300/1,500（6×/5×），per-token 全局统一 | burst/sustained 免费 < 付费 | plan 决定 project 上限和 daily quota |
| **端点加权** | resolveRequestCost（UBA=3, AB=2, CRUD=1） | 多种 query 独立 throttle | cost-based（DSAR=8, GET=1） |
| **跨 token 聚合** | 项目级加权聚合（所有 token 共享 #3） | team-level（所有 team key 之和） | project-level（同 project 的 key 共享） |
| **查询并发保护** | 无（P0） | 查询执行层：per-team=3、per-org=20（Redis ZSET 信号量） | 5–10 并发（API 文档未明确执行层还是 HTTP 层） |
| **错误语义** | 401/403/429/503 已区分 | 401 区分 token 类型，429 含 Retry-After | 401/403/429 已区分 |

> **burst + sustained**：竞品普遍双窗口，本设计第一版先做 burst。原因：(1) Wave 当前 per-token + global 已是单层 burst，增量最小；(2) 上线后 metrics 若显示持续跑满再追加 sustained quota，不影响已有规则。
>
> **保护层级差异**：PostHog 只需 2 层（key → team）的原因是其 token scope 更严格——每个 key 绑定到特定 team，没有"跨 project 叠加"的场景。Wave 的 API token 可访问多项目，因此需要 4 层更细粒度控制。
>
> **查询并发保护**：PostHog 的 per-team=3、per-org=20 并非 HTTP API 层 semaphore，而是**查询执行层**信号量（Redis ZSET + Lua）。详情见 [_research/posthog-research.md §1.5](./_research/posthog-research.md#15-高成本查询rate-与-concurrency-分离)。Wave 的查询同样异步，API 层 semaphore 无效，但查询执行层的并发控制未来可借鉴。
...
| `project_agg_rps` | int | 项目聚合速率（加权） | 50 | 300 | 1,500 |
...
|------|------|-----|------|-----|------|
| Project aggregate | 50/s | 300/s | 6× | 1,500/s | 5× |
...
- 例：PLUS project=50/s，预期最多 5 个 token → token burst ≤ 10/s ✅
