...
| 读侧批量补当前账号名已存在成熟模式 | `apps/web/service/account/account.go:390-408`、`apps/web/op/service/audit.go:128-158` | 不需要写时冗余 actor 快照 |
...
- `snapshot`
  ...
  - `delete` 尽量记录删除前最小摘要；拿不到时至少保留 `id`
- `comment`
  - 可选的人类可读说明
  - 仅放调用点天然已知的信息
- `extra`
...
- `snapshot` 和 `extra` 是 `map[string]any` 类型，任意新增字段无需版本升级
...
- 单条 `detail` 预算：64KB
- 超过 64KB 记 warning，不主动裁剪（PG TOAST 机制自动压缩大字段）
- 如需 Doris 方案再单独评估显式压缩或裁剪策略
...
| --- | --- |
| `created` | 记录当前对象最小摘要 |
| `updated` | 记录 after 摘要，不额外查 before |
| `deleted` | 记录删除前最小摘要；拿不到时只保留 `id` |
...
这对第三方审计是可接受的，因为真正的稳定证据仍然是 `account_id`。
