...
| 公共服务 | `Log` / `BatchLog` / `ListByQuery` | `apps/web/service/activity/service.go` | OP 审计的 `apps/web/op/service/audit.go` |
...
- 动作信息：基础 `action_type` + 可选 `action_name`
- 归因信息：`operator_kind`、`operator_id`、`operator_name`、`source`、`correlation_id`、`occurred_at`，其中 `operator_name` 是展示快照，`occurred_at` 明确定义为操作时间 / 活动事件时间
- 表在 project schema（meta）内，`project_id` 字段冗余，不设此列
- 详情：`detail_version` + `detail_payload`，其中 `detail_version` 是 payload schema version，V1 固定写 `1`，`detail_payload` 使用 JSONB 存稳定 envelope
...
**action_name** — 可选领域动作名，用于表达对象域细语义，如 `online`、`release`、`variant_change`、`relate`、`stop`。它不是新的基础动作层，而是 `action_type` 的补充标签，由活动模块统一注册。

设计要点：
- **守口到活动模块**：一个文件看完所有基础 `action_type` 和已注册 `action_name`
- **基础动作稳定**：`create` / `update` / `delete` / `copy` 是 V1 的固定主枚举
- **领域语义外置**：需要特定语义时新增 `action_name`，不让基础动作枚举持续膨胀
- 内部操作（冲突解决）通常复用相同 `action_type/action_name`，再用 `source="internal"` 区分
...
`web` 和 `openapi` 都代表用户主动操作。`internal` 代表系统行为，但如由用户触发，仍应尽量继承原始用户。`backfill` 用于历史数据迁移，可无真实用户。

为避免把“来源”和“操作者身份”混在一起，建议补充：

```go
type OperatorKind string

const (
    OperatorKindAccount  OperatorKind = "account"
    OperatorKindSystem   OperatorKind = "system"
    OperatorKindBackfill OperatorKind = "backfill"
)
```

`detail_payload` 在 V1 使用 **JSONB 可读 envelope**：

1. SerializeDetail 生成稳定 JSON（如 `{version, changes, extra, snapshot}`）
2. 通过字段投影和大小预算控制 payload
3. 若超限，优先截断明确的大字段，并把 `extra.truncated_fields`、`extra.payload_hash` 写入上下文

V1 不默认启用应用层 LZ4。等真实写入规模证明 JSONB 成为瓶颈后，再评估 codec 升级。
...
ListByQuery(ctx, query) ([]ActivityLogItem, string, error)
```
...
活动写入的一致性不再由调用方自由传参，而是由活动模块内部的 **policy registry** 按 `item_type + action_type + action_name` 中心化决策：
...
核心字段定义为：`item_type`, `item_id`, `action_type`, `operator_kind`, `source`, `occurred_at`（表在 project schema 内，无 `project_id` 列）。
...
V1 推荐在 OP 或等价内部链路提供对象历史查询接口，优先采用 cursor 模型，最小输入输出如下：
...
- 如出现 `online` / `release` / `variant_change` / `relate` / `stop` 等更细语义，均视为 `action_name`
- 因此，表格里若只写了领域动作名，可按“基础 `action_type` + 对应 `action_name`”理解

#### 4.7.1 资产对象

**CHART**

| 场景 | action_type / action_name | 触发来源 | detail.changes[] 最小集合 | extra | 备注 |
|------|------------|---------|--------------------------|-------|------|
...
| 添加 Chart 到 Dashboard | `update` / `relate` | web/openapi/mcp | `chart_ids` before/after | `extra.added_chart_ids` | 记在 Dashboard 活动中；被添加的 Chart 自身不产生活动 |
| 从 Dashboard 移除 Chart | `update` / `unrelate` | web/openapi/mcp | `chart_ids` before/after | `extra.removed_chart_ids` | 同上 |
...
| 状态变更：Debug | `update` / `debug` | web/openapi/mcp | `status` before/after | `extra.conflict_resolution`（若触发了冲突解决） | |
| 状态变更：Online | `update` / `online` | web/openapi/mcp | `status`, `enabled` before/after | `extra.conflict_resolution`, `extra.exposure_property_created` | |
| 状态变更：Offline | `update` / `offline` | web/openapi/mcp | `status`, `enabled` before/after | `extra.buckets_released` | 释放 layer buckets |
| 状态变更：Delete | `delete` | web/openapi/mcp | `status` before/after, 至少 `name` 快照 | `extra.buckets_released`, `extra.references_removed` | |
| Release | `update` / `release` | web/openapi/mcp | `status`, `release_plan` before/after | `extra.release_scope`（流量分配变更摘要） | 涉及流量分配和 bucket 释放 |
| 复制实验 | `copy` | web/openapi/mcp | 可为空 | `extra.source_item_id`, `extra.source_ffkey` | |
| 内部下线（冲突解决）| `update` / `offline` | internal | `status`, `enabled` before/after | `extra.reason: conflict_resolution`, `extra.conflict_ffkey` | 必须记录，source=`internal` |
...
| 状态变更：Debug/Online/Offline | `update` / `debug|online|offline` | web/openapi/mcp | `status`, `enabled` before/after | `extra.conflict_resolution` | |
| 状态变更：Delete | `delete` | web/openapi/mcp | 至少 `name` 快照 | `extra.references_removed` | |
| Release | `update` / `release` | web/openapi/mcp | `status`, `release_plan` before/after | 无 | |
| 复制开关 | `copy` | web/openapi/mcp | 可为空 | `extra.source_item_id` | |
| 内部下线/删除（冲突解决）| `update` / `offline` 或 `delete` | internal | 同 Experiment | `extra.reason: conflict_resolution` | 必须记录 |
...
| 变体变更 | `update` / `variant_change` | web/openapi/mcp | 变体字段 before/after | `extra.changed_variant_keys` | 高价值场景，必须能看出改了哪个 variant |
...
| 停止 Pipeline | `update` / `stop` | web/openapi/mcp | `exec_status` before/after | `extra.stop_reason` | 用户主动停止 |
...
2. **detail 大对象字段裁剪策略**：统一采用“对象投影 -> JSON envelope -> 超限截断并记录 `truncated_fields` / `payload_hash`”三段式；V1 不默认做应用层压缩。
