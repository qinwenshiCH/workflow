...
- Summary：先交付 `object_audit_log + auditlog 模块 + Phase 1 live-write + AB/Metric/Wave 历史迁移 + OP 查询接口`，把“单对象排障闭环”一次打透
- Effort：L
- Risk：Med
- Pros：
  - 直接命中当前最真实的 P0 场景
...
| D-AUTO-12 | Mechanical | 表在 project schema 内，索引为 `(object_type, object_id, created_at DESC)` | project schema 提供天然隔离，不需要 project_id 列 |
...
```text
                 ...
                 │                             │                             │
                 ▼                             ▼                             ▼
      meta.object_audit_log        OP/internal query path         backfill runners
...
1. **索引以 `(object_type, object_id, created_at DESC)` 为主**
2. **项目对象 live-write 由调用方按需传入 consistency 参数**
3. **global / account 继续独立落地，但查询与字段规范向统一 envelope 靠拢**

#### 需要显式写进方案的架构约束

- `object_audit_log` 的权限边界是 project-scoped（由 project schema 天然隔离），不允许跨 schema 查询
...
| `auditlog.Log` | insert `object_audit_log` 失败 | db write error | Y | 直接返回并回滚主事务 | 写操作失败，业务不提交 |
...
|---|---|---|---|
| OP 查询接口越权读取其他项目对象历史 | Med | High | project schema 天然隔离 + OP/internal scope 校验 |
...
1. **对象历史查询索引**：`(object_type, object_id, created_at DESC)` 覆盖 OP 查询主路径
...
- live-write 失败：包含 `object_type`、`object_id`、`action_type`
...
domain services (chart/dashboard/cohort/ab/metadata/account)
    ...
    │        │
    │        ▼
    │   object_audit_log DAO
    │
    └── OP query service ───▶ object_audit_log read

backfill runners ───────────▶ legacy sources (AB / metric history) + object_audit_log write
```
...
- `object_audit_log` 读路径：索引 `(object_type, object_id, created_at DESC)` 覆盖主查询
...
- [ ] **T1 (P1, human: ~1 day / CC: ~20min)** — audit storage — 建 `object_audit_log` 表及索引 `(object_type, object_id, created_at DESC)`
  - Surfaced by: CEO Section 1 / Eng Architecture — 表在 project schema 内，project_id 冗余不设列
  - Files: `specs/20260626-Wave-Feat-AddAuditLog/plan.md`, future `dao/auditlog` migration
  - Verify: explain 输出索引命中；查询走 (object_type, object_id) 主路径
...
- [ ] **T5 (P1, human: ~0.5 day / CC: ~10min)** — OP query — 明确 project scope auth、masked readback、decode failure fallback
  ...
  - Verify: project schema 隔离；detail decode 失败时返回基础字段 + unavailable 标记
...
- **Theme: completeness beats convenience** — per-call consistency、project schema 隔离、full-phase tests 都是同一价值判断
