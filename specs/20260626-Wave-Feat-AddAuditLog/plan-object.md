...
- 主键：`id`
- 主查询键：`(item_type, item_id, created_at DESC)`
- 活动主体：`item_type`、`item_id`、`item_name`，其中 `item_name` 是展示快照
- 归因信息：`operator_id`、`operator_name`、`source`、`created_at`，其中 `operator_name` 是展示快照，`created_at` 明确定义为操作时间 / 活动事件时间
...
**action_type** — 操作类型，全小写描述性字符串。**所有常量统一定义在 `activity/types.go`**，模块不自行定义。各模块按需使用已有常量或在此文件注册新常量（PR review gate 确保语义不重叠）。
...
设计要点：
- **守口到活动模块**：一个文件看完所有 action_type，查询端可直接按这些值过滤
...
活动写入的一致性不是系统级策略，而是**每个调用方在调用时按需决定的参数**。`Log`/`BatchLog` 接口接受一个 `consistency` 参数：

```go
type ActivityConsistency int

const (
    // 核心字段+detail 都失败则回滚业务事务
    ConsistencyStrong ActivityConsistency = iota
    // 核心字段失败则回滚，detail 失败允许降级（写空/截断+warning）
    ConsistencyCore ActivityConsistency = iota
    // 失败只记 warning，不阻塞业务
    ConsistencyBestEffort ActivityConsistency = iota
)
...
核心字段定义为：`item_type`, `item_id`, `action_type`, `operator_id`, `source`, `created_at`（表在 project schema 内，无 `project_id` 列）。
...
这样设计让一致性策略透明可追溯，避免”一刀切强一致性过于刚硬、整个 best-effort 又怕关键操作丢记录”的两难。
...
- 索引以 `(item_type, item_id, created_at DESC)` 结尾，partition pruning 可直接利用
- 未来需要 TTL 清理时，按 `created_at` 月份分区，直接 drop 旧分区即可
- V1 上线后根据实际写入速率评估是否需要分区，不在首批实现

global 管理活动继续沿用 `LogWithFallback`，这条策略**不复用到项目对象标准活动**。详见 [plan-org.md](./plan-org.md)。
...
推荐每个对象类型都注册一个 `ActivityProjectionBuilder`，输出稳定 map：
...
| 添加 Chart 到 Dashboard | `relate` | web/openapi/mcp | `chart_ids` before/after | `extra.added_chart_ids` | 记在 Dashboard 活动中；被添加的 Chart 自身不产生活动 |
...
1. **活动一致性等级**：调用方在 `Log`/`BatchLog` 调用时按需传入 `consistency` 参数决定。API 提供 `Strong` / `Core` / `BestEffort` 三级。活动域不替调用方做选择，但各模块应在接入时显式声明等级。
