...
- 记录所有实体的**管理面操作（created/updated/deleted）** + 登录/登出事件
- **不干涉各业务内部状态流转**（AB 状态变更、Metric 口径变更、MA Campaign 启动/暂停等属于产品功能）
- 账号活跃字段方案废弃，登录/登出事件直接走审计日志
...
- 实体分类使用 `domain + feature` 两列替代单一 `scope`：
  ...
  - `feature`：细粒度实体类型（共 20 个，每个 domain 下 1:1 或 1:N 映射）
- 完整 domain/feature 清单（21 个 entity），对齐 Wave 内部领域模型：

  | domain | feature | 对应实体 |
  | --- | --- | --- |
  | `account` | `auth` | Account（登录/登出） |
...
- `action` 使用全小写字符串，基础动作集锁定为 `created / updated / deleted / logged_in / logged_out`
  - 不含 read / view 等读操作
...
- `global.audit_log` 表结构：
  ...
  | `feature` | VARCHAR(64) | NOT NULL | 细粒度实体类型，如 auth / chart / experiment |
...
- **Phase 0（底座）**：`global.audit_log` 建表（含分区）+ GORM 插件 + Middleware + 写入/查询模块 + 分区管理脚本
- **Phase 1（高价值对象接入）**：domain=account(auth)、organization(org/member/invite)、project(project/member)、asset(chart/dashboard/cohort)
- **Phase 2（长尾对象接入）**：domain=account(token)、asset(experiment/feature_gate/feature_config/pipeline/campaign)、metadata(metric)
