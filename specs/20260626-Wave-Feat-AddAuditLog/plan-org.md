...
| **客户侧管理审计** | `global.audit_log`（新表） | 客户侧操作：组织创建/归档、项目创建/删除、成员管理、权限同步 |

分界原则：OP 人员在 OP 后台的操作走 `op_operation_log`；客户（含组织管理员）在业务系统中的操作走 `audit_log`。OP 未来通过内部接口在同一页面展示两类日志。

字段集 = `meta.audit_log` + `org_id` + `project_id` + `detail_version`/`detail_payload`。action_type 使用 audit_log 自有枚举（create/update/delete），与 meta.audit_log 不共用 const 块。

邀请流程：邀请建在自有表上。接受邀请 → 触发生效操作 → 写入 `audit_log`。邀请本身（创建/发送/撤回）不落审计。
...
CREATE INDEX idx_audit_org
    ON audit_log(org_id, object_type, created_at DESC);
CREATE INDEX idx_audit_project
    ON audit_log(project_id, object_type, created_at DESC);
...
- `action_type` 在 `auditlog/types.go` 中以 audit_log 自有 const 块定义（create/update/delete），不与 meta.audit_log 共享枚举
- `detail_version` 与 `meta.audit_log` 复用同一版本号空间，V1 固定为 1
...
- 邀请创建/发送/撤回 — 邀请建在自有表上，接受邀请后触发生效操作走 audit_log
...
**detail_payload** 存储为 BYTEA、LZ4 压缩。大小约束与 `meta.audit_log` 对齐：单条序列化后最大 64KB，超出时优先 LZ4 压缩；压缩后仍超则逐字段截断并记录警告。写入时由 AuditService 自动完成序列化和压缩，调用方直接传入 Detail 结构体即可。
...
- 添加成员 → `audit_log` 出现 `action_type=create, object_type=ORG_MEMBER`
