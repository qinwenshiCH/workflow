...
CREATE INDEX idx_activity_org
    ON activity_log(org_id, item_type, created_at DESC);
CREATE INDEX idx_activity_project
    ON activity_log(project_id, item_type, created_at DESC);
CREATE INDEX idx_activity_item
    ON activity_log(item_type, item_id, created_at DESC);
CREATE INDEX idx_activity_operator
    ON activity_log(operator_id, created_at DESC);
```

- `action_type` 在 `activity/types.go` 中以 activity_log 自有 const 块定义（create/update/delete），不与 meta.activity_log 共享枚举
...
活动服务层提供两种写入方法，由调用方按场景选择：

- **`Log(ctx, input)`** — 强一致性。写入失败返回 error，调用方决定是否回滚业务。适用于合规关键路径。
- **`LogWithFallback(ctx, input)`** — best-effort。写入失败记日志 + 递增指标，不影响业务。适用于排障场景。

**管理活动推荐**：`LogWithFallback`（主价值是排障，不是合规锁）。但此选择权在调用方，不在 plan 层硬编码。

**批量操作约束**：单次 `BatchInsert` 不超过 500 行（见 DAO 节约束），避免大事务锁表。

**detail_payload** 存储为 BYTEA、LZ4 压缩。大小约束与 `meta.activity_log` 对齐：单条序列化后最大 64KB，超出时优先 LZ4 压缩；压缩后仍超则逐字段截断并记录警告。写入时由 ActivityService 自动完成序列化和压缩，调用方直接传入 Detail 结构体即可。

## 6. 查询接口

新增 OP 查询端点，按 org 维度拉取该客户侧的所有管理活动记录，供 OP 内部排障使用。
...
svc.WriteManagementLogWithFallback(ctx, ManagementWriteInput{
    OrgID:      orgID,
    ItemType: "ORG_MEMBER",
    ObjectID:   accountID,
...
2. 部署新代码（DAO + 8 个接入点），活动写入推荐 `LogWithFallback`（调用方可按场景选择），失败不影响业务
3. OP 查询接口上线
...
- `ListByOrg` / `ListByProject` / `ListByItem` / `ListByOperator` 分页正确
- OP 查询权限：非管理员被拒绝
