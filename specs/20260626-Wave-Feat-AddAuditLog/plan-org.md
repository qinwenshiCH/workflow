...
| **客户侧管理审计** | `global.mgmt_audit_log`（新表） | 客户侧操作：组织创建/归档、项目创建/删除、成员管理、权限同步 |

分界原则：OP 人员在 OP 后台的操作走 `op_operation_log`；客户（含组织管理员）在业务系统中的操作走 `mgmt_audit_log`。OP 未来通过内部接口在同一页面展示两类日志。

字段集 = `object_audit_log` + `org_id` + `project_id` + `detail_version`/`detail_payload`。action_type 使用 mgmt_audit_log 自有枚举（create/update/delete），与 object_audit_log 不共用 const 块。

邀请流程：邀请建在自有表上。接受邀请 → 触发生效操作 → 写入 `mgmt_audit_log`。邀请本身（创建/发送/撤回）不落审计。
...
CREATE INDEX idx_mgmt_audit_org
    ON mgmt_audit_log(org_id, object_type, created_at DESC);
CREATE INDEX idx_mgmt_audit_project
    ON mgmt_audit_log(project_id, object_type, created_at DESC);
CREATE INDEX idx_mgmt_audit_object
    ON mgmt_audit_log(object_type, object_id, created_at DESC);
CREATE INDEX idx_mgmt_audit_operator
    ON mgmt_audit_log(operator_id, created_at DESC);
```

- `action_type` 在 `auditlog/types.go` 中以 mgmt_audit_log 自有 const 块定义（create/update/delete），不与 object_audit_log 共享枚举
- `detail_version` 与 `object_audit_log` 复用同一版本号空间，V1 固定为 1
- `idx_mgmt_audit_project` 支撑"查某个 project 的成员变更"场景
- `idx_mgmt_audit_operator` 支撑"某操作人最近对组织做了什么"的排障路径
...
- 邀请创建/发送/撤回 — 邀请建在自有表上，接受邀请后触发生效操作走 mgmt_audit_log
...
新增 `apps/web/dao/mgmt_audit_log.go`（~70 行），复用 `globaldb.TableDao` 模式：
...
`ListByProject` 利用 `idx_mgmt_audit_project` 索引，支撑项目维度审计查询。
...
1. migration 先跑 → `mgmt_audit_log` 表就位
...
- 添加成员 → `mgmt_audit_log` 出现 `action_type=create, object_type=ORG_MEMBER`
