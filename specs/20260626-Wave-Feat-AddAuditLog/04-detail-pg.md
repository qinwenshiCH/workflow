...
| `pkg/config/app_cfg.go` | 修改 | 新增 audit 配置项（batch/flush/queue） | 审计模块需要可配置 |
...
COMMENT ON COLUMN audit_log.event_id IS '稳定事件标识，用于回放幂等和导出对账';
COMMENT ON COLUMN audit_log.source IS '来源：ui / api_token / mcp';
COMMENT ON COLUMN audit_log.detail IS 'JSON 字符串：{schema_version, account, target, comment, extra}';
COMMENT ON COLUMN audit_log.ip_address IS '操作者 IP 地址，合规刚需';
COMMENT ON COLUMN audit_log.occurred_at IS '事件实际发生时间，由调用方传入';
COMMENT ON COLUMN audit_log.created_at IS '入库时间，异步场景下可能晚于 occurred_at';
```
...
| `audit_log_queue_size` | `int` | `4096` | channel 容量，满时非阻塞丢弃并记 counter + error 日志，不设 spool | ❌ |
| `audit_log_detail_max_bytes` | `int` | `65536` | detail 大小预算 (64KB)，超限丢弃 detail 并打 warn 日志 | ❌ |
| `audit_log_export_max_rows` | `int` | `100000` | 单次导出最大行数 | ❌ |
...
| gin `ClientIP()` | 框架 | gin 路由 IP 提取 | `c.ClientIP()`，X-Real-IP 由 APISIX 透传 | gin v1.x | 框架提供 | 无 TrustedProxies 配置时 IP 可能为容器 IP；V1 文档说明此限制 |
| `net/http.Request` | 框架 | MCP 路由 IP 提取 | 读取 `X-Real-IP`（APISIX 透传），降级读 `RemoteAddr` | Go stdlib | 框架提供 | 网关丢帧时降级值不准确 |
| `pvctx` | 内部模块 | 鉴权与审计上下文 | 函数调用 | 当前版本 | 内部 | 需新增 `audit_source` 与 `client_ip` |

> **IP 提取策略**：只信任 APISIX 网关透传的 `X-Real-IP`。不自行解析 `X-Forwarded-For`，不配置全局 `TrustedProxies`（影响所有 gin 路由，超出审计范围）。其他来源（`RemoteAddr`、容器 IP）仅在网关丢帧时降级使用。V1 文档说明此限制，IP 可能在某些场景下不准确。
...
| **OrganizationFilter** | **扩大范围注入 `org_id`**，不限 API Token 请求 | `pkg/ginx/middleware/organization.go` |
...
flowchart TD
    ...
    Check -->|否| Skip1[静默跳过<br/>+ warn log]

    Validate -->|合法| CheckSize[64KB 大小预算检查]
    Validate -->|非法| Skip2[静默跳过<br/>+ warn log]
...
>
> **org_id 来源说明**：`audit.Log()` 内部从 `pvctx.OrgID(ctx)` 提取 org_id。账号层事件（§10.1）ctx 中无 OrgID，写 NULL。组织/项目层事件（§10.2–§10.5）由 OrganizationFilter 中间件统一注入——该中间件已扩大范围，不再仅限于 API Token 请求。MCP 由 `authorizeProjectContext` 注入。各 handler 无需手动处理 org_id。
>
> 代码形态统一为：
> ```go
...
|---|---|---|---|---|---|---|
| 1 | `session` | `logged_in` | `controller/account/account.go` | `Login` | 空 | Account: `{id}`, 无 Target |
| 2 | `session` | `logged_out` | `controller/account/account.go` | `Logout` | 空 | Account: `{id}`, 无 Target |
| 3 | `session` | `login_failed` | `controller/account/account.go` | `Login`（失败路径） | 空 | Account: `{id}`（可识别时）或 `{name: "登录名"}`（仅标识），不记录密码 |
| 4 | `account_setting` | `updated` | `controller/account/account.go` | `UpdateProfile` / `ChangePassword` / `ChangeEmail` | account ID | Target: `{id, name, type:"account"}` |
| 5 | `api_token` | `created` | `controller/account/api_token.go`（推测） | `CreateToken` | token ID | Target: `{id, name, type:"api_token"}` |
| 6 | `api_token` | `updated` | 同上 | `UpdateToken` / `RevokeToken` | token ID | Target: `{id, name, type:"api_token"}` |
| 7 | `api_token` | `deleted` | 同上 | `DeleteToken` | token ID | Target: `{id, name, type:"api_token"}` |

### 10.2 Domain: organization

| # | Feature | Action | Controller | Handler | targetID | Detail 要点 |
|---|---|---|---|---|---|---|
| 8 | `org_setting` | `updated` | `controller/org/org.go` | `UpdateOrg` | org ID | Target: `{id, name, type:"organization"}` |
| 9 | `org_member` | `created` | `controller/org/member.go` | `AddMember` | 成员 account ID | Target: `{id, name, type:"org_member", role}` |
| 10 | `org_member` | `updated` | 同上 | `UpdateMemberRole` | 成员 account ID | Extra: `{role_before, role_after}` |
| 11 | `org_member` | `deleted` | 同上 | `RemoveMember` | 成员 account ID | Target: `{id, name, type:"org_member"}` |
| 12 | `org_member_invitation` | `created` | `controller/org/invitation.go` | `InviteMember` | invitation ID | Target: `{id, email_hint, type:"org_invitation"}` |
| 13 | `org_member_invitation` | `deleted` | 同上 | `CancelInvitation` | invitation ID | Target: `{id, type:"org_invitation"}` |

### 10.3 Domain: project

| # | Feature | Action | Controller | Handler | targetID | Detail 要点 |
|---|---|---|---|---|---|---|
| 14 | `project_setting` | `updated` | `controller/project/project.go` | `UpdateProject` | project ID | Target: `{id, name, type:"project"}` |
...
| # | Feature | Action | Controller | Handler | targetID | Detail 要点 |
|---|---|---|---|---|---|---|
| 18 | `chart` | `created` | `controller/chart/chart.go` | `CreateChart` | chart ID | Target: `{id, name, type:"chart", ...filtered}` |
| 19 | `chart` | `updated` | 同上 | `UpdateChart` | chart ID | Target: `{id, name, type:"chart", ...filtered}` |
| 20 | `chart` | `deleted` | 同上 | `DeleteChart` / `BatchDeleteChart` | chart ID(s) | Extra: `{ids: [...]}`（批量时） |
| 21 | `dashboard` | `created` | `controller/dashboard/dashboard.go` | `CreateDashboard` | dashboard ID | Target: `{id, name, type:"dashboard"}` |
| 22 | `dashboard` | `updated` | 同上 | `UpdateDashboard` / `PatchMeta` / `SetLayouts` / `AddCharts` / `RemoveCharts` | dashboard ID | Extra: `{chart_ids: [...]}` |
| 23 | `dashboard` | `deleted` | 同上 | `DeleteDashboard` / `BatchDeleteDashboard` | dashboard ID(s) | Extra: `{ids: [...]}`（批量时） |
| 24 | `cohort` | `created` | `controller/cohort/cohort.go` | `CreateCohort` | cohort ID | Target: `{id, name, type:"cohort"}` |
...
| 27 | `experiment` | `created` | `controller/experiment/experiment.go` | `CreateExperiment` | experiment ID | Target: `{id, name, type:"experiment"}` |
| 28 | `experiment` | `updated` | 同上 | `UpdateExperiment` | experiment ID | Target: `{id, name, type:"experiment"}` |
| 29 | `experiment` | `deleted` | 同上 | `DeleteExperiment` | experiment ID | Target: `{id, name, type:"experiment"}` |
| 30 | `feature_gate` | `created` | `controller/feature/feature_gate.go` | `CreateFeatureGate` | flag ID | Target: `{id, name, type:"feature_gate"}` |
| 31 | `feature_gate` | `updated` | 同上 | `UpdateFeatureGate` | flag ID | Target: `{id, name, type:"feature_gate"}` |
| 32 | `feature_gate` | `deleted` | 同上 | `DeleteFeatureGate` | flag ID | Target: `{id, name, type:"feature_gate"}` |
| 33 | `feature_config` | `created` | `controller/feature/feature_config.go` | `CreateFeatureConfig` | config ID | Target: `{id, name, type:"feature_config"}` |
| 34 | `feature_config` | `updated` | 同上 | `UpdateFeatureConfig` | config ID | Target: `{id, name, type:"feature_config"}` |
| 35 | `feature_config` | `deleted` | 同上 | `DeleteFeatureConfig` | config ID | Target: `{id, name, type:"feature_config"}` |
| 36 | `pipeline` | `created` | `controller/pipeline/pipeline.go` | `CreatePipeline` | pipeline ID | Target: `{id, name, type:"pipeline"}` |
| 37 | `pipeline` | `updated` | 同上 | `UpdatePipeline` | pipeline ID | Target: `{id, name, type:"pipeline"}` |
| 38 | `pipeline` | `deleted` | 同上 | `DeletePipeline` | pipeline ID | Target: `{id, name, type:"pipeline"}` |
| 39 | `tracking_plan` | `created` | `controller/tracking_plan/tracking_plan.go` | `CreateTrackingPlan` | plan ID | Target: `{id, name, type:"tracking_plan"}` |
| 40 | `tracking_plan` | `updated` | 同上 | `UpdateTrackingPlan` | plan ID | Target: `{id, name, type:"tracking_plan"}` |
| 41 | `tracking_plan` | `deleted` | 同上 | `DeleteTrackingPlan` | plan ID | Target: `{id, name, type:"tracking_plan"}` |
| 42 | `layer` | `created` | `controller/ab/layer.go` | `CreateLayer` | layer ID | Target: `{id, name, type:"layer", parent_experiment_id}` |
| 43 | `layer` | `updated` | 同上 | `UpdateLayer` | layer ID | Target: `{id, name, type:"layer"}` |
| 44 | `layer` | `deleted` | 同上 | `DeleteLayer` | layer ID | Target: `{id, name, type:"layer"}` |
| 45 | `holdout` | `created` | `controller/ab/holdout.go` | `CreateHoldout` | holdout ID | Target: `{id, name, type:"holdout", parent_experiment_id}` |
| 46 | `holdout` | `updated` | 同上 | `UpdateHoldout` | holdout ID | Target: `{id, name, type:"holdout"}` |
| 47 | `holdout` | `deleted` | 同上 | `DeleteHoldout` | holdout ID | Target: `{id, name, type:"holdout"}` |
| 48 | `target` | `created` | `controller/ab/target.go` | `CreateTargetRule` | target ID | Target: `{id, name, type:"target", parent_experiment_id}` |
| 49 | `target` | `updated` | 同上 | `UpdateTargetRule` | target ID | Target: `{id, name, type:"target"}` |
| 50 | `target` | `deleted` | 同上 | `DeleteTargetRule` | target ID | Target: `{id, name, type:"target"}` |

### 10.5 Domain: metadata

| # | Feature | Action | Controller | Handler | targetID | Detail 要点 |
|---|---|---|---|---|---|---|
| 51 | `metric` | `created` | `controller/metric/metric.go` | `CreateMetric` | metric ID | Target: `{id, name, type:"metric"}` |
| 52 | `metric` | `updated` | 同上 | `UpdateMetric` | metric ID | Target: `{id, name, type:"metric"}` |
| 53 | `metric` | `deleted` | 同上 | `DeleteMetric` | metric ID | Target: `{id, name, type:"metric"}` |
| 54 | `tracked_event` | `created` | `controller/event/tracked_event.go` | `CreateTrackedEvent` | event ID | Target: `{id, name, type:"tracked_event"}` |
...
| 57 | `virtual_event` | `created` | `controller/event/virtual_event.go` | `CreateVirtualEvent` | event ID | Target: `{id, name, type:"virtual_event"}` |
...
| 60 | `event_property` | `created` | `controller/property/event_property.go` | `CreateEventProperty` | property ID | Target: `{id, name, type:"event_property"}` |
...
| 63 | `user_property` | `created` | `controller/property/user_property.go` | `CreateUserProperty` | property ID | Target: `{id, name, type:"user_property"}` |
...
| 66 | `virtual_property` | `created` | `controller/property/virtual_property.go` | `CreateVirtualProperty` | property ID | Target: `{id, name, type:"virtual_property"}` |
...
func (h *ChartHandler) UpdateChart(ctx, id, req) (res, err) {
    ...
    audit.Log(ctx, audit.DomainAsset, audit.FeatureChart, audit.ActionUpdated,
        fmt.Sprint(chart.ID), &audit.Detail{
            Account: map[string]any{"id": pvctx.AidStr(ctx)}, // name 天然已知时也可填
...
func (h *DashboardHandler) BatchDeleteDashboards(ctx, ids) (res, err) {
    ...
    audit.Log(ctx, audit.DomainAsset, audit.FeatureDashboard, audit.ActionDeleted,
        fmt.Sprint(ids[0]), &audit.Detail{
            Account: map[string]any{"id": pvctx.AidStr(ctx)},
