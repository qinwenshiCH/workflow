...
flowchart LR
    ...
    Purger --> MAClient["MA Purge Client"]
    MAClient --> MA["MA Runtime owner"]
...
| 2 | `project_ma` | 确认项目消费组无 active member，再清共享/独享 Redis 和消费组 |
| 3 | `project_wagent` | 确认 execution ownership 已释放，再定向清 Stream/PEL/DLQ 和项目 Key |
| 4 | `project_redis` | 确认 Scheduler ownership 已释放，再清 Scheduler 与 Web Redis 状态 |
| 5 | `project_oss` | 清四类 Wave 管理前缀并确认空 |
| 6 | `project_kafka` | 确认项目 Topic 无 active assignment，再清 Topic/专属消费组并核验 |
| 7 | `project_doris` | 取得 migration mutex并重查 `PURGING`，再 Drop 项目 Database |
| 8 | `project_data_pg` | 取得 migration mutex并重查 `PURGING`，再 Drop Data Schema |
| 9 | `project_meta_pg` | Asset 有界 drain 后 Drop Meta Schema，项目 Job 随 Schema 删除 |
| 10 | `final_global` | 最终核验后在短事务中清引用和敏感字段，提交 `PURGED,true` |
...
- Dispatch/C1、MA、LiveEvent 查询 task map 或 Kafka group member/assignment；
...
- Redis、Wagent、OSS、MA 数据只在对应 owner 的上述核验成功后清理。
...
Web 不读取 MA 私有 Redis，也不在 Web 内实现 MA 清理：

```mermaid
sequenceDiagram
    participant P as ProjectResourcePurger
    participant C as MAPurgeClient
    participant H as MA Internal HTTP
    participant R as MA Runtime

    P->>C: PurgeProject(ctx, projectID)
    C->>H: POST /internal/v1/project/purge
    H->>H: 校验专用 Secret、caller 和 Project header
    H->>R: PurgeProject(ctx, projectID)
    R->>R: 清共享/独享 Redis 与项目消费组
    R->>R: 确认资源不存在
    R-->>H: success
    H-->>C: 204
    C-->>P: project_ma 完成
...
MA 多副本不做逐 Pod fan-out：所有持久资源是共享 Redis/Kafka，任一健康实例都能完成删除；各 Pod 的进程内工作由 PM Delete Hook 和 Scheduler heartbeat 收敛。旧 MA 未部署 endpoint 时返回非成功，Web 保持 `PURGING`，不得跳过 `project_ma`。
...
| Edge/C1/MA/QE/LiveEvent/Asset | 真实项目内存状态驱逐 | 中 |
| Redis/OSS/Kafka/Doris/PG owner | 幂等删除、资源不存在核验、防隐式重建 | 高 |
| MA internal HTTP | Web 到 MA 私有资源的单一清理入口 | 中 |
...
3. 执行 organization status migration，部署全部 MA 实例的内部 endpoint 和共享 Secret。
...
- MA endpoint 尚未全量时不得开放 Web Purge；非 204 只能让项目停在 `PURGING`。
