...
| Project Purge（新数据） | `DISABLE/INITIALIZING,false -> PURGING,false -> PURGED,true` | 保持不存在 | 删除 |
...
1. 可用项目集合索引、项目运行时快照和变更发布的关键 Redis 错误返回调用方。
...
- Redis、Wagent、OSS、MA 数据在对应 writer 静默后清理。
