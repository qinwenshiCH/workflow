...
| 文件 | 改动 |
| --- | --- |
| `web/service/liveevent/liveevent.go`、`dal/kafkax` | 再次幂等 `CloseProject`；Kafka Admin 确认 `live-event-<pid>-*` group 无 active member 后删除并核验，仍活动或查询失败时停止 Purge |
...
| 共享 Redis | execution/compaction Stream、PEL、DLQ | 可恢复队列 | 全局 Stream entry 携带 `project_id`，由 consumer group 领取和恢复；项目状态只能在消息解码后检查 | `apps/web/wagent/service/runtime/queue.go`、`compaction.go` | XREADGROUP / XCLAIM / XPENDING |
| 项目 Redis | execution/lease/event/active-lock/marker Key | 运行资源 | 协调 execution ownership、事件和入队去重 | `apps/web/wagent/service/runtime/` | executionKey / activeLockKey |
...
flowchart LR
    subgraph enable["ENABLE"]
        ...
        subgraph e_redis["项目 Redis"]
            EQueue["execution/compaction Stream、PEL、DLQ"]
    ...
    subgraph disable["DISABLE"]
        ...
        subgraph d_redis["项目 Redis"]
            DQueue["execution/compaction Stream、PEL、DLQ"]
    ...
    subgraph purged["PURGED"]
        ...
        subgraph p_redis["项目 Redis"]
            PQueue["无目标项目 Stream/PEL/DLQ entry"]
    ...
    EQueue -->|Delete：解码后拒绝应用层 claim；不 ACK/XDEL| DQueue
...
> 当前代码 claim/heartbeat 不查 PM。项目 ID 位于 Stream entry 内，因此门禁只能放在 `XREADGROUP/XCLAIM` 解码后、应用层 `claimExecution`/handler 前；项目不可用时不 ACK、不 XDEL。
...
sequenceDiagram
    ...
    participant S as Redis Stream / PEL
    participant E as Executor running map
    participant H as Heartbeat

    alt ENABLE
        W->>S: XREADGROUP / XCLAIM
        S-->>W: 返回含 project_id 的消息
        ...
        W->>S: XREADGROUP / XCLAIM 后解码
        Note over W,S: PM 不含项目→不 claimExecution、不 ACK、不 XDEL
        H->>E: heartbeat 续约
        Note over H: 查 PM 发现项目不存在<br/>→ cancel context、release lease
        deactivate E
        Note over S: 消息保留在 Stream/PEL<br/>Restore 后可恢复
    else PURGED
...
| 文件 | 改动 |
| --- | --- |
| `web/wagent/service/runtime/local_executor.go`、`compaction.go` | `XREADGROUP/XCLAIM` 解码后、应用层 claim/handler 前检查 PM；项目不可用时不 ACK、不 XDEL、不启动 |
...
| 文件 | 改动 |
| --- | --- |
| `web/wagent/service/runtime/queue.go`、`compaction.go` | 从现有 Stream/PEL 恢复领取 |

#### Purge 阶段

| 文件 | 改动 |
| --- | --- |
| `web/wagent/service/runtime/` | 先用现有 execution ownership/pending heartbeat确认 handler 已退出；再按 `project_id` 分页清 Stream、PEL、DLQ entry 和精确项目 Key，无法确认时返回 `project_wagent` |
...
| Kafka | producer | 持久资源 | 将采集到的数据写入 Kafka，实现数据从入口到处理系统的异步传输 | `apps/edge/service.go` | kafkax.NewProducer / kafka.Writer |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | 代码文件 | 搜索关键词 |
| --- | --- | --- | --- | --- | --- |
| Meta PG | AB 配置 | 持久资源 | 存储项目的 AB 实验定义和参数配置，支撑实验判定和流量分配 | `apps/abol/service/internal_meta_loader.go` | GetInternalABConfigs |
| 项目 Redis | target cache | 运行资源 | 缓存 AB 实验目标查找结果，加速实验判定决策 | `apps/abol/service/abol.go` | tarCache |
...
| Kafka | C1 extractor group | 持久资源 | 记录项目消费位点，支撑断点续传和 Exactly-Once 语义 | `apps/c1/model/config.go`、`apps/c1/extractor/extractor.go` | ExtractorKafkaGroupID |
...
flowchart LR
    ...
    DTasker -->|Purge：权威 map/assignment 已无项目| PTasker
...
| `c1/metadata/metadata.go` | `OnProjectDelete` 只驱逐 metadata/quota 映射；consumer/Pipeline 由 Dispatch owner 停止，全局 loader 不按项目关闭 |
...
| 文件 | 改动 |
| --- | --- |
| `service/project/resource_purger.go` | `project_dispatch` 核验全部 active host task map 不含目标项目；`project_kafka` 确认共享 C1 group assignment 无目标 Topic 后删除 Topic，不删除整个 C1 group |
...
| 客户外部 | 目标系统 | 持久资源 | 客户拥有的外部数据目标，Connector 仅转发数据、不管理其生命周期 | `apps/connector/service/event2webhook_stream.go` | WebHook / target |
...
flowchart LR
    ...
    DRunner -->|Purge：Scheduler ownership 已释放| PRunner
...
| 存储位置 | 资源 | 类型 | 内容与作用 | 代码文件 | 搜索关键词 |
| --- | --- | --- | --- | --- | --- |
| Meta PG | campaign、audience 等表 | 持久资源 | 存储营销活动的受众定义、触达配置和元数据，支撑 MA 执行引擎决策 | `apps/ma/service/dispatch/` | campaign / audience |
...
| MA 独享 Redis | `ma:{p:<pid>}:*` | 持久资源 | 存储 MA 项目独享的运行状态数据（延迟队列、幂等/合并键、限速、续传位点），支撑营销自动化的独立执行 | `apps/ma/service/delayqueue/`、`apps/ma/service/dispatch/stores.go` | ma:{p: |
| 共享 Redis | `ma:{p:<pid>}:*`（epoch、fanout rate-limit） | 持久资源 | 存储 MA 项目在多副本间共享的运行数据（pub/sub、activation epoch），支撑跨实例协调 | `apps/ma/service/eventconsumer/coordinator.go`、`apps/ma/service/fanout/ratelimit.go` | ma:{p: |
| Kafka | MA 消费组 `{GroupPrefix}.{pid}` | 持久资源 | 记录 MA 项目事件消费的偏移量，保证营销触达的可靠性和顺序执行 | `apps/ma/service/eventconsumer/coordinator.go` | GroupPrefix / group(pid) |
| Scheduler | Instance/Task/lease | 运行资源 | 调度 MA 的定时 Campaign 执行和周期任务，由 Scheduler Worker 管理生命周期 | `pkg/scheduler/worker.go` | RegisterHandler |
...
flowchart LR
    ...
    DJob -->|Purge：Scheduler ownership 已释放| PJob
...
| 文件 | 改动 |
| --- | --- |
| `ma/service/purge.go` **新增** | 内部 endpoint `POST /internal/v1/project/purge`：确认配置的项目消费组无 active member，再清独享/共享 Redis和 exact group |


**MA Runtime Purge 顺序**（endpoint 内部）：
1. 对当前进程执行幂等本地驱逐（再次确认 Hook 已执行）
2. 查询配置的 MA 项目消费组；仍有 active member 时失败
3. 删除共享 Redis `ma:{p:<pid>}:*`
4. 删除 MA 独享 Redis `ma:{p:<pid>}:*`
5. 删除 exact MA 项目消费组并重新查询两个 Redis 和 Kafka；全部不存在才返回成功
...
| 3 | `project_wagent` | Wagent | 确认 execution ownership 已释放，再按 `project_id` 清 Stream/PEL/DLQ 和项目 Key |
...
| 6 | `project_kafka` | Kafka owner | 确认项目 Topic 无 active assignment，再删除 Topic 和 Wave-owned group |
...
| Web 旁路 | MCP/Internal/QE、LiveEvent active group、Asset drain、Wagent Stream/PEL 行为 |
