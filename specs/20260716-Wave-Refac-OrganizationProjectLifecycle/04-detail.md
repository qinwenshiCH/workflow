...
| `dao/global/project.go` | 新增 `UpdateStatusIf`（条件 UPDATE + RowsAffected 检查）；普查已有 `is_deleted` 查询，生产路径加 `status IN ('ENABLE','INITIALIZING')` 条件 |
...
| 文件 | 改动 |
| --- | --- |
| `pkg/pm/project_manager.go` | 关键 Redis 错误上抛；写后本地同步；`autoSubscribe` 断线后指数退避重连 + 重连后调 `loadAllProjects()` 全量对账（复用已有方法） |
