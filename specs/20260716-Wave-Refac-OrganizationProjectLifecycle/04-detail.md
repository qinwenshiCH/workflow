...
- Delete/Restore 只修改 Global PG 状态和 PM 可用目录；Delete 可触发资源 owner 停止项目运行资源并驱逐必要的进程内状态，但不删除 Scheduler Job、Schema、Topic、OSS、成员或业务 Redis。
...
flowchart LR
    ...
    subgraph Runtime["运行平面"]
        ...
        AsyncHook["异步组件直接接入模块<br/>Connector / MA"]
        Scheduler["pkg/scheduler"]
        AsyncWorker["Scheduler Handler<br/>Connector / MA"]
    ...
    Global --> PM
    PM -->|"直接查询或 Hook"| Entry
    PM -->|"直接查询或 Hook"| WebRun
    PM -->|"直接查询或 Hook"| AsyncHook
    PM -->|"统一任务门禁"| Scheduler
    PM -->|"直接 Hook"| Dispatch
    Scheduler -->|"间接受控"| AsyncWorker
    Dispatch -->|"间接受控"| C1
    WebCtl --> Stores
    Entry --> Stores
    WebRun --> Stores
...
| 层次 | 判断标准 | 生命周期责任 |
| --- | --- | --- |
| app/进程初始化 | 入口进程调用 `pm.InitManager` | 持有一个 PM 实例和本地项目快照；不代表进程内所有模块自动受控 |
| 模块直接接入 | 模块查询 PM 或注册 `ProjectInfoHooker` | 入口拒绝不可用项目，或响应 Update/Delete Hook |
| 模块间接受控 | 模块的工作由 Scheduler 或 Dispatch 创建和持有 | 由共享门禁、heartbeat 或 topology 刷新停止和恢复 |

当前接入关系和资源 owner 按 `app/包 → 模块` 排列；"进程初始化 PM"与"模块拥有项目资源"必须分列评审：

| app/包 | 模块 | 进程初始化 PM | 当前 PM 接入或间接控制 | 项目运行资源或进程内状态 |
| --- | --- | --- | --- | --- |
| `apps/web` | ProjectService / OP 生命周期 | 是 | OP 入口调用 Service；Service 调用 `PM.SetInfo/DeleteInfo` 写可用项目目录 | 不持有项目运行资源 |
| `apps/web` | 普通 Web API / `ProjectFilter` | 是 | 每次请求查询本地 PM | 只有单次请求 context |
| `apps/web` | MCP / `authorizeProjectContext` | 是 | 当前只校验成员关系和 Token scope，未查询 PM | 只有单次 tool context |
| `apps/web` | Internal S2S | 是 | 当前 `InternalProjectContext` 只解析 Project Header | 入口本身不持有资源；调用下游 Pipeline、MA、AB 资源 |
| `apps/web` | QE Catalog | 是 | 已注册 `ProjectInfoHooker` | Catalog、MetaCache、refresh lock |
| `apps/web` | LiveEvent | 是 | 建连时查询 PM，未注册有效 owner Hook | WebSocket、Kafka consumer、连接 map |
| `apps/web` | Asset Behavior | 是 | 由普通 Web 入口间接控制，模块未直接接入 PM | 项目 batcher、goroutine、buffer |
| `apps/web` | Wagent runtime | 是 | HTTP 创建入口间接受控；后台 worker 当前不查询 PM | execution、compaction、Stream claim context |
| `apps/web` | Migration runner | 是 | 不以 PM 为唯一来源，使用数据库项目查询 | Meta/Doris migration 执行 context |
| `apps/web` | 权限与 Token cache | 是 | 普通请求间接受控；cache 本身不查询 PM | 项目权限、资产权限、Project→Org、Token scope cache |
| `apps/edge` | Router | 是 | 每次请求调用 `Token2ProjectID` | 只有单次采集请求 |
| `apps/edge` | TrackService | 是 | 已注册 `ProjectInfoHooker` | token 路由、pipeline version、internal secret map |
| `apps/adtol` | Router | 是 | 每次请求调用 `Token2ProjectID` | 无长期项目运行资源或进程内状态 |
| `apps/abol` | Router | 是 | 每次请求查询 PM | 只有单次 AB 请求 |
| `apps/abol` | Abol core | 是 | 已注册 `ProjectInfoHooker` | `abCore`、metadata loop |
| `apps/c1` + `pkg/dispatch` | Dispatch Node / TaskManager | 是 | Node 注册 Hook；TaskManager 由 topology 间接受控 | Redis task map、Tasker、consumer、loader |
| `apps/c1` | Metadata | 是 | 查询 PM quota | metadata、quota 等项目 map |
| `apps/connector` | HTTP Router | 是 | AppsFlyer 等入口调用 `Token2ProjectID` | 只有单次请求 |
| `apps/connector` | Pipeline runtime | 是 | 注册 Hook；handler 由 Scheduler Worker 间接控制 | Pipeline handler、Kafka runner/consumer、批导上下文 |
| `apps/ma` | ConfigSync | 是 | 已注册 `ProjectInfoHooker` | view、tracked project、subscription |
| `apps/ma` | Runtime / Scheduler handler | 是 | 部分 runtime 枚举 PM；handler 由 Scheduler Worker 间接控制 | consumer、watcher、matcher、feedback/config/cohort cache |
| `apps/simulator` | 生产生命周期 | 否 | 无 | 无生产 Wave 项目资源 |
| `pkg/pm` | Manager | 由所在进程初始化 | 维护本地项目快照并调用各模块 Hook | `projects` map、Hook 集合、Pub/Sub subscription |
| `pkg/scheduler` | Master | 由所在 app 初始化 | 当前只有 reload/repair 等部分路径枚举 PM | cron、notify、待生成的 Instance |
| `pkg/scheduler` | Worker | 由所在 app 初始化 | Instance/Task 领取与 heartbeat 尚未统一检查 PM | 运行中 handler、ownership、lease |

同一 app 中未查询 PM、未注册 Hook、也不经 Scheduler/Dispatch 的模块不会自动收到生命周期变化；后续组件台账逐项判断它是否拥有需要收敛的项目资源。

**PM Delete Hook 与运行收敛缺口**

只有模块持有项目级内存、连接、consumer、goroutine 等资源，并且不能通过入口门禁、Scheduler heartbeat 或 Dispatch topology 自动收敛时，才需要 PM Delete Hook。下表仍按 `app/包 → 模块` 排列，只汇总覆盖缺口，不替代或删减第 4.5–4.10 节的资源台账。

| app/包 | 模块 | Delete 后可能残留 | 当前收敛方式 | 是否需要 Hook | 本期目标 |
| --- | --- | --- | --- | --- | --- |
| `apps/web` | ProjectService / OP 生命周期 | 无本地运行资源 | 调用 `PM.SetInfo/DeleteInfo` 改变可用目录 | 否 | 保持生命周期控制职责，不直接处理模块本地资源 |
| `apps/web` | 普通 Web API / `ProjectFilter` | 已越过门禁的单次请求 | 请求完成或 context 超时 | 否 | 保留统一入口门禁，不增加逐请求 Hook |
| `apps/web` | MCP / `authorizeProjectContext` | 已启动的单次 tool context | context 完成或超时 | 否 | 在统一授权函数补 PM 门禁，不增加逐 tool Hook |
| `apps/web` | Internal S2S | 新工作命令及既有执行回调 | 当前只解析 Project Header | 否 | Service 入口区分新工作和在途收尾，不增加全局 Hook |
| `apps/web` | QE Catalog | Catalog、MetaCache、refresh lock | 已注册 Hook，但 `OnProjectDelete` 为空 | 是，补全现有 Hook | 驱逐目标项目状态；Restore 后查询懒加载 |
| `apps/web` | LiveEvent | WebSocket、Kafka consumer、连接 map | 新连接检查 PM，已有连接不收敛 | 是，新增 owner Hook | 关闭目标连接和 consumer，并从 map 移除 |
| `apps/web` | Asset Behavior | batcher、goroutine、buffer | 当前不监听 PM Delete | 是，新增 owner Hook | 有界 drain/flush 后关闭并移出项目 map |
| `apps/web` | Wagent runtime | execution、compaction | 当前后台 claim/heartbeat 不查询 PM | 否 | 由 claim/start 门禁和 heartbeat 取消收敛 |
| `apps/web` | Migration runner | Meta/Doris migration context | 数据库查询决定项目集合，不依赖 PM Delete 收敛 | 否 | `DISABLE,false` 继续升级；`PURGING/PURGED` 不再创建 migration |
| `apps/web` | 权限与 Token cache | 被动 TTL cache | 不会自行创建工作 | 否 | Delete 保留；Purge 按固定 namespace 定向清理 |
| `apps/edge` | Router | 已越过门禁的单次采集请求 | 每次请求查询 PM | 否 | 保持现有入口门禁 |
| `apps/edge` | TrackService | token/config 派生 map | 已注册 Hook，但 `OnProjectDelete` 为空 | 是，补全现有 Hook | 驱逐 `token2id`、`pipelineVersion`、`internalSecrets` |
| `apps/adtol` | Router | 无长期项目资源 | 每次请求查询 PM | 否 | 保持现有入口门禁 |
| `apps/abol` | Router | 已越过门禁的单次 AB 请求 | 每次请求查询 PM | 否 | 保持现有入口门禁 |
| `apps/abol` | Abol core | `abCore`、metadata loop | 现有 Hook 停止并删除目标 core | 已覆盖 | 保持实现并补回归测试 |
| `apps/c1` + `pkg/dispatch` | Dispatch Node / TaskManager | counts、Redis task map、Tasker | Hook 只删除 counts，removed project 未可靠触发 topology 重写 | 是，修正现有 Hook 链路 | 标记 topology changed、重写 task map 并关闭 Tasker |
| `apps/c1` | Metadata | metadata、quota 等项目 map | 当前没有完整 owner Hook | 是，新增 owner Hook | 驱逐目标项目 metadata map |
| `apps/connector` | HTTP Router | 已越过门禁的单次 AppsFlyer 请求 | 每次请求查询 PM | 否 | 保持现有入口门禁 |
| `apps/connector` | Pipeline runtime | handler、Kafka runner/consumer、批导 context | Delete Hook 为空；实际工作由 Scheduler 持有 | 否 | 不增加第二套停止逻辑；统一由 Scheduler 门禁和 heartbeat 收敛 |
| `apps/ma` | ConfigSync | view、tracking、subscription | `Untrack` 能清理自身资源 | 已覆盖自身资源 | 保留 `Untrack`，不把它当作整个 MA 已覆盖 |
| `apps/ma` | Runtime / Scheduler handler | consumer、watcher、matcher、feedback/config/cohort cache | Scheduler 只覆盖 handler，其他 runtime 未统一收敛 | 仅本地 owner 状态需要 | handler 用 Scheduler heartbeat；其余由 MA Runtime owner Hook 驱逐 |
| `apps/simulator` | 生产生命周期 | 无生产项目资源 | 无 | 否 | 不增加空 Hook |
| `pkg/pm` | Manager | 调用节点本地 `projects` map、远端陈旧快照 | 当前依赖 Pub/Sub 回环；断线后无完整差异对账 | 不适用，负责触发 Hook | 调用节点 write-through；远端重订阅后双快照对账 |
| `pkg/scheduler` | Master | 新 Instance/Task | notify、已有 cron 等路径未统一检查 PM | 否 | notify、cron 和生成 Instance 前统一门禁 |
| `pkg/scheduler` | Worker | 运行中 handler、ownership、lease | 领取和 heartbeat 尚未统一检查 PM | 否 | 领取门禁和 heartbeat 取消统一收敛 |

"注册了 Hook"只表示模块能收到通知，不表示它已经关闭了对应资源；空实现或只处理部分资源仍属于覆盖缺口。反过来，已有 heartbeat、topology 或入口门禁能够可靠收敛的模块不增加空 Hook。

#### 4.0.3 生命周期影响矩阵

| 动作 | 生命周期控制状态 | 可用性平面 | 运行平面 | 项目业务数据 |
| --- | --- | --- | --- | --- |
| Delete | `ENABLE,false → DISABLE,false` | `PM.DeleteInfo` | 入口拒绝新工作；真实资源 owner 驱逐必要进程内状态；Scheduler/Dispatch 收敛运行任务 | 不删除、不扫描、不改写 |
...
| 职责分组 | app/包 | 模块/范围 | 明细 |
| --- | --- | --- | --- |
| 生命周期控制与项目存储入口 | `apps/web` | ProjectService、普通 Web API、MCP、Internal S2S、QE、LiveEvent、Asset、Wagent、权限/cache | 4.5 |
| 在线入口 | `apps/edge`、`apps/adtol`、`apps/abol` | Router、TrackService、Abol core | 4.6 |
| 数据与异步执行 | `apps/c1`、`apps/connector`、`apps/ma` | Dispatch/Metadata、HTTP/Pipeline、ConfigSync/Runtime | 4.7 |
| 独立工具 | `apps/simulator` | 生产生命周期 | 4.8 |
| 共享运行骨架 | `pkg/pm`、`pkg/scheduler`、`pkg/dispatch`、存储 client | Manager、Master/Worker、Node/TaskManager、清理 owner | 4.9–4.10 |
...
#### 4.1.1 Project/Organization/OP 资源台账
...
本节描述本期改造后的目标边界；当前代码已覆盖与缺口见 4.2.4，具体实现文件见第 5 章。
...
> Asset Behavior（原 4.5.6）是 Web 进程内的 goroutine manager（`apps/web/service/asset/behavior.go`），不拥有独立数据存储，生命周期绑定进程。Per-project Delete 通过 PM Hook drain/flush/Close batcher，合并到 Web 运行资源收敛处理。此处不再独立列台账和 Mermaid 图。
...
| `入口 → PM 门禁` | 4.2.4 的普通 Web、MCP、Internal S2S、在线服务、Scheduler、Dispatch/C1、Wagent、组件本地运行资源和 Migration 例外均有当前事实、目标检查点与测试 |
| `运行资源 → 收敛方式` | 每个 consumer、goroutine、WebSocket、batcher、Tasker、handler 都能回指 Delete Hook、heartbeat、topology、context 取消或明确的在途完成边界 |
...
| `apps/web/service/project/delete.go` | 轻量生命周期 | 删除 `Archive` 和旧重 Delete；实现 `ProjectService.Delete/Restore`；Restore 用已有配置调用 `PM.SetInfo`，不调用资源初始化 |
...
| --- | --- | --- |
| `apps/web/server.go`、`pkg/ginx/middleware/project.go` | 审核全部生成路由和 `projectWhitelist`；保留普通项目 API 的 PM 门禁，删除旧租户 lifecycle 入口；OP 生命周期继续明确绕过 | 无资源清理 |
...
| `pkg/internalauth/middleware.go` | `InternalProjectContext` 继续只解析 Header，不增加全局 PM 拦截 | 无资源清理 |
| `apps/web/service/pipeline/internal_metadata.go` | 新工作调用 `requireInternalProjectEnabled`；在途查询与 finish/update/advance/complete 在 PM 缺失时读取 Global 生命周期，只允许 `DISABLE,false` 更新既有执行 | 无资源清理 |
| `apps/web/ma/service/*` | materialize 等新工作入口检查 PM；结果回写在 PM 缺失时读取 Global 生命周期，只允许 `DISABLE,false` 更新既有执行 | 无资源清理 |
| `apps/web/qe/catalog/catalog.go` | Delete Hook 删除项目 Catalog；Restore 懒加载 | 清两类 QE refresh lock |
| `apps/web/controller/liveevent/liveevent.go`、`apps/web/service/liveevent/liveevent.go` | 保留 `RegisterClient` 的 PM 门禁；Delete Hook 关闭项目 consumer/WebSocket 并移出 map；Restore 新连接懒启动 | Kafka owner 删除残留临时 group |
...
| `apps/web/wagent/service/runtime/*` | execution/compaction claim、start 和 heartbeat 检查 PM；Delete 消息不 ACK、不 XDEL，运行中 execution 取消 | `PurgeProject` 清项目 Stream/DLQ、`p:<pid>:*` |
...
| 包/文件 | 具体改动 |
| --- | --- |
| `pkg/pm/project_manager.go` | 可用项目集合索引、项目运行时快照和变更发布的关键 Redis 错误上抛；写后本地同步、订阅断开重连和双快照对账；保留 `SetInfo/DeleteInfo`，不新增 Restore 事件或方法 |
...
5. ProjectService 不直接调用任何资源 owner；`PM.DeleteInfo` 只让已注册的本地 owner Hook 收敛进程内状态，Scheduler/Dispatch 通过共享路径收敛运行任务。
6. 不扫描 Redis；业务 Redis 保留，task map、lease 等派生运行状态只在各自共享 owner 中定向重写、释放或自然过期。
...
`Restore` 是 ProjectService 的业务方法，PM 不增加 `RestoreInfo/OnProjectRestore`。不调用 `InitProjectResources`，不等待 Hook 或 Scheduler 收敛，不检查/重建持久资源；各模块从 PM 当前快照经 Update Hook、Scheduler/Dispatch 或懒加载恢复。
...
| 顺序 | step | 操作 | 失败影响 |
| --- | --- | --- | --- |
| 0 | `org_precondition` | 全部子项目 `PURGED,true` | 有活跃项目则返回 Conflict |
| 1 | `org_quiescence` | 确认无活跃写入引用该组织 | 保留 `PURGING`，下次重试 |
| 2 | `org_global` | 最终事务清邀请、组织成员、role、token scope 中的组织 ID 引用 | 事务失败，保留 `PURGING` |
| 3 | `org_expire_customer` | 调用既有 `ExpireCustomerByOrgID` | 业务补偿（客户绑定非核心） |
| 4 | `org_verify` | 最终核验引用已全部清除 | 保留 `PURGING` 状态 |

最终事务保留 Account、Customer Profile、合同、审计和子项目墓碑。不操作项目级基础设施资源。
...
PM 传播的是“项目当前可用/不可用”状态，不传播 Create/Restore 等历史动作。Create、普通配置更新和 Restore 都可调用 `SetInfo`；新启动或重连进程只需要根据当前快照执行 `OnProjectUpdate`，不依赖曾经是否收到 Restore 事件。
...
本节只汇总改造完成后的运行规则；当前代码事实、缺口和具体检查位置以 4.2.4 为准，不能据本表推断当前已经接入。
...
`InternalProjectContext` 继续只解析 Header，因为同组存在无项目 admin endpoint；不在 middleware 全局阻断所有内部请求。PM 缺失时，在途接口读取 Global 生命周期，只允许 `DISABLE,false` 的既有执行收尾；`PURGING/PURGED/is_deleted=true/NotFound` 全部拒绝。运行面静默后才删除底层资源。
...
| 流量入口覆盖 | 4.2.4 每一行均验证 PM 缺失时不会进入业务 handler、创建/领取工作或创建长生命周期资源；Internal 在途收尾与 Migration 例外单独验证 |
...
- [x] 第 4 章原有资源台账、生命周期图和适配结论未删减，只补充流量入口与收敛链路。
- [x] 第 4.2 节覆盖表使用一致的 `app/包 → 模块` 名称和顺序，可横向对应 PM 接入、Hook 缺口、流量入口与收敛方式。
- [x] 当前代码事实与目标入口门禁分开描述，可逐入口证明 Delete 后不再创建新工作。
