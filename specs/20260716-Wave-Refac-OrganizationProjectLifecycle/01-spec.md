...
- Purge 只拒绝 `ENABLE` 项目；接受 `DISABLE/INITIALIZING/PURGING`，其中历史 `DISABLE/PURGING,true` 保持 `is_deleted=true`。`PURGED,true` 重复调用直接返回已完成。
...
- `INITIALIZING/PURGING/PURGED` 与历史 `DISABLE,true -> PURGING,true -> PURGED,true` 的 Restore/Purge 边界。
