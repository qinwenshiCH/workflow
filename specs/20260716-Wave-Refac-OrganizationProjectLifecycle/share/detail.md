...
- **运行子模块**：QE Catalog、LiveEvent、Asset Behavior、Wagent（2.4–2.7）
- **共享缓存**：权限、Token cache（2.8）
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Global PG | project/org 主记录 | 持久资源 | ID、归属、配置、`status/is_deleted` 权威状态 | 写 `DISABLE` | 写 `ENABLE` | 写 `PURGED,true` | `dao/global/project.go:57`、`dao/global/organization.go:39` |
| Global PG | member、邀请引用 | 持久资源 | 成员关系和未完成邀请 | 保留 | 保留 | 最终事务移除 | `dao/global/project_member.go:344`、`dao/global/member_invite.go:59` |
| PM Redis | 可用项目集合索引 `sys:{pm}:projects` | 运行资源 | 可用 project ID 集合，供组件枚举 | 移除 project ID | 写回 project ID | 确认不含 | `pkg/pm/project_manager.go:79` |
| PM Redis | 项目运行时快照 `sys:{pm}:info:<pid>` | 运行资源 | Secret、状态、Schema/Database/Topic、配额 | 删除快照 | 写回现有快照 | 确认不存在 | `pkg/pm/project_manager.go:213,234` |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Project PG | Meta `schema_<pid>` | 持久资源 | 事件/属性、Scheduler、AB、Dashboard、Pipeline 等表 | 保留 | 不检查、不重建 | `DROP SCHEMA CASCADE` | `pkg/dal/pgsqlx/` |
| Project PG | Data `schema_<pid>` | 持久资源 | `id_relation`、`raw_users` 身份关系 | 保留 | 不检查、不重建 | `DROP SCHEMA CASCADE` | `pkg/dal/pgsqlx/` |
| Doris | 项目 Database | 持久资源 | 事件、用户、分群、统计、MA fanout | 保留 | 不检查、不重建 | `DROP DATABASE IF EXISTS` | `pkg/dal/dorisx/` |
| Kafka | 项目 Topic 集合 | 持久资源 | `raw_event/event/other/error` 四类数据流 | 保留 | 不检查、不重建 | 删除 Topic + consumer group | `pkg/dal/kafkax/` |
| OSS | 项目前缀 | 持久资源 | `load/backfill/events_cron/users_cron/<pid>/` | 保留 | 不检查、不重建 | 删除四类前缀并确认空 | `pkg/dal/ossx/` |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 进程内存 | `catalogs[projectID]` | 进程内状态 | 事件、属性、Cohort、Metric 查询 | Hook 驱逐 | 懒加载 | 清除 | `apps/web/qe/catalog/catalog.go:53` |
| 进程内存 | MetaCache | 进程内状态 | QE Catalog 响应缓存 | Hook 驱逐 | 懒加载 | Meta Schema 删除 | `apps/web/qe/catalog/catalog.go:252` |
| 项目 Redis | refresh lock Key | 运行资源 | 防止重复 refresh | 保留 | 继续使用 | `project_redis` 删除 | `apps/web/qe/catalog/notifier.go:212-213` |
...
| 现状 | 本期改动 |
| --- | --- |
| 已注册 `ProjectInfoHooker`，`OnProjectDelete` 为空 | 补全：驱逐 `catalogs[projectID]` 和 MetaCache；Restore 查询时懒加载 |
| refresh lock 项目级保留 | 保持保留策略；Purge 时清两类 QE refresh lock |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 进程内 | WebSocket | 运行资源 | 实时事件推送 | Hook 关闭 | 新连接懒启动 | 已无连接 | `apps/web/service/liveevent/liveevent.go:94` |
| 进程内 | Kafka consumer | 运行资源 | 消费项目事件 | Hook 关闭 | 新连接重建 | 等待关闭 | `apps/web/service/liveevent/liveevent.go:223` |
| Kafka | `live-event-<pid>-*` group | 持久资源 | broker group metadata | 保留 | 不主动重建 | 按前缀删除 | `apps/web/service/liveevent/liveevent.go:223` |

**PM 接入现状与代码改动**

| 现状 | 本期改动 |
| --- | --- |
| 未注册 PM Delete Hook | 新增 owner Hook：遍历 `connections[projectID]` 关闭 WebSocket + consumer，移出 map |
| 建连时查 PM | 保留 `RegisterClient` 的 PM 门禁 |
| — | Restore 不做操作（新连接懒启动） |
| — | Purge：Kafka owner 删除残留 consumer group |

### 2.6 Asset Behavior

**定位**

Asset Behavior（`apps/web/service/asset/behavior.go`）是 Web 进程内的 goroutine manager。它为每个项目维护一个 batcher，批处理资产操作。不拥有独立数据存储，生命周期绑定进程。

**流量入口与任务**

Asset 工作由普通 Web API 触发，但项目请求进入 batcher 后通过 goroutine 异步处理，不受入口门禁约束。项目 Delete 后 batcher 可能继续处理已缓冲的数据。

**PM 接入现状与代码改动**

| 现状 | 本期改动 |
| --- | --- |
| 未直接接入 PM | 新增 PM Delete Hook：`CloseProject` → drain/flush 后移出 `batchers[projectID]` |
| — | Restore：不重建 batcher（下一请求懒创建） |
| — | Purge：无额外持久资源需清理 |

> Asset Behavior 不拥有独立数据存储，生命周期绑定进程。其资源变动已在 2.1 生命周期变化中体现（drain/flush/Close batcher），不单独列台账。
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Meta PG | `wagent_conversation/message` | 持久资源 | Conversation、Message 查询 | 保留 | 继续使用 | Meta Schema Drop | `apps/web/wagent/service/conversation.go:49,115` |
| 项目 Redis | execution/compaction Stream、DLQ、pending | 持久资源 | 入队或失败重试 | 禁止 claim/start，不 ACK | 恢复后重新领取 | 定向删除 entry | `apps/web/wagent/service/runtime/execution.go:51` |
| 项目 Redis | lease/event/active-lock Key | 运行资源 | 互斥、租约、事件 | 保留或自然过期 | 继续使用 | 定向删除 | `apps/web/wagent/service/runtime/events.go:175` |
| 项目 Redis | quota/rate-limit Key | 运行资源 | 配额和限流 | 保留或自然过期 | 继续使用 | 定向删除 | `apps/web/wagent/service/tokenquota/service.go:811`、`apps/web/wagent/service/ratelimit/ratelimit.go:146` |
| 进程内存 | executor `running` map | 进程内状态 | 运行中 execution 管理 | heartbeat 取消 | 新执行创建 | 清除 | `apps/web/wagent/service/runtime/local_executor.go:25` |
| 进程内存 | MCP tool TTL cache | 进程内状态 | 工具列表缓存 | 保留或 TTL | 新请求恢复 | 不单独清理 | `apps/web/wagent/service/` |
...
| 现状 | 本期改动 |
| --- | --- |
| 后台 claim/heartbeat 不查 PM | claim/start 前检查 PM，不存在的项目不领取 |
| — | heartbeat 查 PM，不含项目时取消 context、释放 lease |
| — | Delete 消息不 ACK、不 XDEL（保留在 Stream 中） |
| — | Restore 后重新领取 |
| — | Purge：清项目 Stream/DLQ、`p:<pid>:*`、quota/rate-limit Key（`wagent/service/runtime/*`、`wagent/service/tokenquota`、`wagent/service/ratelimit`） |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 进程内存 | `token2id`、`pipelineVersion`、`internalSecrets` | 进程内状态 | Token 路由、Pipeline 版本、Secret 查询 | Hook 驱逐 | Hook 写回 | 不处理 | `apps/edge/service.go:37-39` |
| Kafka | producer | 持久资源 | 采集数据写入 | 保留 | 不变 | 不处理（Edge 不拥有 Kafka Topic） | `apps/edge/service.go` |
...
| 现状 | 本期改动 |
| --- | --- |
| 已注册 `ProjectInfoHooker`，`OnProjectDelete` 为空 | 补全：驱逐 `token2id`、`pipelineVersion`、`internalSecrets` map |
| `Token2ProjectID` 每请求查 PM | 保留现有门禁 |
| — | 验证现有 producer 默认不自动建 Topic |
...
| 现状 | 本期改动 |
| --- | --- |
| Router 每请求 `Token2ProjectID` 查 PM | 保留现有门禁 |
| — | 增加 Delete/Restore 回归测试 |
| — | 不新增空 Hook |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Meta PG | AB 配置 | 持久资源 | AB 实验配置 | 保留 | 继续读取 | Meta 清理 | `apps/abol/service/internal_meta_loader.go:21` |
| 项目 Redis | target cache | 运行资源 | AB Core target lookup | 保留 | 继续使用 | Redis 清理 | `apps/abol/service/abol.go:22` |
| 进程内 | `abCore[projectID]` | 进程内状态 | ABOL project core lookup | Hook 删除 | Hook 重建 | 清除 | `apps/abol/service/abol.go:28` |
| 进程内 | metadata loop | 运行资源 | metadata 同步 | Hook 停止 | Hook 重建 | 停止 | `apps/abol/service/internal_meta_loader.go:21` |
...
| 现状 | 本期改动 |
| --- | --- |
| 已注册 Hook，`OnProjectDelete` 停止并删除 ABCore，`OnProjectUpdate` 重建 | 保持现有实现，补回归测试 |
| Router 每请求查 PM | 保留门禁 |
| — | 不改持久资源 |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Redis（Dispatch） | task map `sys:{dispatch}:task:*` | 运行资源 | 项目 task 计数 | Hook 删除 + topo 重写 | 重建 | 删除 | `pkg/dispatch/rdb.go:63` |
| 进程内存 | C1 metadata/quota 映射 | 进程内状态 | 项目 Metadata 和 Quota 查询 | Hook 驱逐 | 查询重建 | 驱逐 | `apps/c1/metadata/metadata.go:48-51` |
| 进程内存 | Dispatch per-project ITasker | 运行资源 | 项目 task 管理 | topo 重写后关闭 | topo 重建 | 关闭 | `pkg/dispatch/dispatcher.go:28` |
| Kafka | consumer/loader | 运行资源 | 项目数据消费 | topo 重写后停止 | topo 重建 | 停止 | `apps/c1/` |
...
| 现状 | 本期改动 |
| --- | --- |
| Dispatch Node 已注册 Hook，但 `OnProjectDelete` 只删 counts | 修正：标记 topology changed + 重写 task map + 关闭 ITasker |
| C1 Metadata 无完整 owner Hook | 新增 Hook：驱逐 `metadata[projectID]` 和 `quota[projectID]` |
| Kafka producer 默认允许自动建 Topic | 设置 `AllowAutoTopicCreation=false`；Topic 缺失时进入错误日志和重试路径 |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Meta PG | Pipeline 配置和运行记录 | 持久资源 | Pipeline 定义、执行日志 | 保留，拒绝新工作 | 继续读取 | 随 Meta Schema | `apps/connector/service/pipeline.go:36` |
| 客户外部 | 目标系统 | 持久资源 | 客户自有系统 | 保留 | 保留 | 不处理 | — |
...
| 现状 | 本期改动 |
| --- | --- |
| HTTP Router 每请求查 PM | 保留门禁 |
| 注册了 Hook 但为空 | 不补全——实际工作由 Scheduler 间接控制，统一由 Scheduler 门禁和 heartbeat 收敛 |
| — | 客户外部目标明确排除在 Purge 范围外 |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 进程内存 | ConfigSync view/tracking/subscription | 进程内状态 | 项目配置同步跟踪 | Hook 清理 | Hook 重建 | 内部 endpoint | `apps/ma/service/configsync/sync.go:43` |
| 进程内存 | Runtime feedback/config/cohort cache | 进程内状态 | 反馈、配置、队列查询 | Hook 驱逐 | Hook 重建 | 内部 endpoint | `apps/ma/service/eventconsumer/consumer.go:79` |
| 进程内存 | Runtime consumer/watcher/matcher | 运行资源 | 事件消费、变化监听 | Scheduler heartbeat | Scheduler 重建 | 内部 endpoint | `apps/ma/service/eventconsumer/consumer.go:96` |
| MA 独享 Redis | `ma:p:<pid>:*` | 持久资源 | 项目运行数据 | 保留 | 保留 | 内部 endpoint 删除 | `apps/ma/service/eventconsumer/` |
| 共享 Redis | `ma:{p:<pid>}:*` | 持久资源 | 项目共享运行数据 | 保留 | 保留 | 内部 endpoint 删除 | `apps/ma/service/eventconsumer/` |
| Kafka | MA 消费组 | 持久资源 | broker group metadata | 保留 | 保留 | 内部 endpoint 删除 | `apps/ma/service/eventconsumer/` |
...
| 现状 | 本期改动 |
| --- | --- |
| ConfigSync 已注册 Hook，`Untrack` 可清理自身 | 纳入统一本地 Delete Hook |
| Runtime 部分枚举 PM，部分无 Hook | Runtime 统一注册 PM Delete Hook |
| Scheduler handler 由 heartbeat 间接控制 | 保留，不增加第二套停止逻辑 |
| — | **新增内部 endpoint** `POST /internal/v1/project/purge`，MA Runtime 清理独享 Redis + 共享 Redis + 消费组 |
| — | Web 通过 `net/http` 调用该 endpoint，不直接访问 MA Redis |
...
| 存储位置 | 资源 | 类型 | 内容与作用 | Delete | Restore | Purge | 代码位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Redis | 可用项目集合索引 `sys:{pm}:projects` | 运行资源 | Set 结构，供组件枚举 | 移除 ID | 写回 ID | 确认不含 | `pkg/pm/project_manager.go:79` |
| Redis | 项目运行时快照 `sys:{pm}:info:<pid>` | 运行资源 | Secret、状态、配置 JSON | 删除 | 写回 | 确认不存在 | `pkg/pm/project_manager.go:213,234` |
| Redis | `sys:{pm}:info_change` Pub/Sub | 运行资源 | 变更通知 | 发布变更 | 发布变更 | — | `pkg/pm/project_manager.go:363` |
| 进程内存 | Manager `projects` map | 进程内状态 | 本地项目快照 | Pub/Sub 或对账更新 | Pub/Sub 或对账更新 | — | `pkg/pm/project_manager.go:79` |
