...
- 不能只依赖 PM Hook：MCP 根路由、Internal S2S API、LiveEvent 长连接、Wagent Redis Stream Worker，以及 Scheduler 已加载 cron/notify 和 Worker 均存在需要单独门禁的路径。
...
- MA 运行态同时使用共享和独享 Redis，项目 Key 形如 `ma:{p:<pid>}:*`，Web 没有独享 Redis 的连接配置；完整 Purge 需要由 MA 自己通过窄的内部接口同步清理两个 Redis，不能在 Delete Hook 中删除。
...
- MA 除 ConfigSync view 外还有按项目的 cohort index/watcher、event matcher、feedback 内存队列和 token cache；现有 ConfigSync `OnProjectDelete` 只做 `Untrack`，不足以驱逐这些本地状态。
...
- Wagent MCP tool list cache 的 key 包含 project ID，但它只是进程内 TTL cache，不能自行产生工作；新执行门禁足以使 Delete 项目不可用，无需为它新增 PM Hook。
